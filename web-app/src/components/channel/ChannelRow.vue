  ...
  <div
    ...
    :class="isMobile
      ...
      : 'group/row grid border-t border-border/60 bg-card transition-colors'
          + ' grid-cols-[2rem_10rem_7rem_minmax(14rem,22%)_1fr_4.5rem_2.5rem] items-center'"
  >
    <!-- Desktop: Drag handle -->
    ...
    <!-- Desktop: Gassenturm -->
    <div v-if="!isMobile" class="border-l border-border/40 h-full flex items-center justify-center px-1">
      <button
        v-if="ch.gassenturmAssignment"
        :title="gassenturmBadge"
        class="no-print flex flex-col items-center justify-center gap-0 rounded-sm px-1 py-0.5 text-accent bg-accent/10 hover:bg-accent/20 transition-colors"
        @click.stop="openModal"
      >
        <span class="font-mono text-[10px] font-semibold leading-none">{{ ch.gassenturmAssignment.turmname }}</span>
        <span class="font-mono text-[9px] text-accent/70 leading-none">Pos. {{ ch.gassenturmAssignment.position }}</span>
      </button>
      <button
        v-else
        title="Gassenturm zuweisen"
        class="no-print size-6 flex items-center justify-center rounded-sm transition-colors text-muted-foreground/25 opacity-0 group-hover/row:opacity-100 hover:bg-muted/40 hover:text-muted-foreground"
        @click.stop="openModal"
      >
    ...
    <div v-if="isMobile" class="flex flex-col gap-0 py-0 px-0 w-full">
      ...
      <!-- Row 3: Notes + Gassenturm badge -->
      <div class="text-xs px-3 py-1.5 border-t border-t-border/30 border-l-2 border-l-border/40 flex items-center gap-2">
        <ChannelTextarea
          v-model="ch.notes"
          :data-nav-row="rowIndex"
          data-nav-col="3"
        ...
        <button
          v-if="ch.gassenturmAssignment"
          class="shrink-0 rounded-full bg-accent/10 px-1.5 py-0.5 text-[10px] font-semibold text-accent border border-accent/20"
          @click.stop="openModal"
        >{{ gassenturmBadge }}</button>
        <button
          v-else
          class="shrink-0 rounded-full bg-muted/40 px-1.5 py-0.5 text-[10px] text-muted-foreground border border-border/40"
          @click.stop="openModal"
        ><TowerControl class="size-3 inline" /></button>
      </div>
    </div>
  </div>
...
const emit = defineEmits([
  ...
  'toggleStatus', 'delete', 'insertAfter', 'updateTower',
])
...
function onSave(assignment) {
  emit('pushSnapshot')
  props.ch.gassenturmAssignment = assignment
...
function onRemove() {
  emit('pushSnapshot')
  props.ch.gassenturmAssignment = null
  modalOpen.value = false
  emit('change')
}

const gassenturmBadge = computed(() => {
  const a = props.ch.gassenturmAssignment
  if (!a?.turmname) return ''
  return `${a.turmname} · Pos. ${a.position ?? '?'}`
})
